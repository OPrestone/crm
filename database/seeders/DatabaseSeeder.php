    ...
    public function run(): void
    {
        ...
        foreach ($contactData as $i => [$first, $last, $title, $score, $source, $status]) {
            $contacts[] = Contact::create([
                ...
                'notes'       => "Met at industry event. Interested in enterprise plan. Follow up scheduled.",
        ...
        foreach ($dealData as $i => [$title, $cIdx, $coIdx, $value, $prob, $days, $status, $priority, $stageIdx]) {
            $deals[] = Deal::create([
                ...
                'status'              => $status,
                'priority'            => $priority,
                'notes'               => "Deal in progress. Next step: send proposal and schedule executive call.",
                'assigned_to'         => $users[$i % count($users)]->id,
                'created_by'          => $owner->id,
            ]);
        }
        ...
        foreach ($taskData as $i => [$title, $type, $priority, $status, $dueDays]) {
            $tasks[] = Task::create([
                'tenant_id'    => $tid,
                'title'        => $title,
                'description'  => "Task: {$title}. Ensure all stakeholders are informed and action items are logged.",
                'type'         => $type,
                'status'       => $status,
                'priority'     => $priority,
        ...
        foreach ($cardSeeds as [$contact, $template, $name, $title, $company, $email, $phone, $website, $address, $linkedin, $twitter]) {
            $data = array_filter([
                ...
                'address' => $address, 'linkedin' => $linkedin, 'twitter' => $twitter,
            ], fn($v) => $v !== null);
            Card::create([
                'tenant_id'   => $tid,
        ...
        $notifData = [
            ...
            [$owner,   'success', 'Deal Won!',                   'BlueWave Starter Pack has been marked as Won — $22,500.',     'trophy-fill',            'success', '/deals',   '-5 hours'],
